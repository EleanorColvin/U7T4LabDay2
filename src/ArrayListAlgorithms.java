...
public class ArrayListAlgorithms {
    ...
    public static void wackyVowels(ArrayList<String> wordList)
    {
        for (int i = 0; i < wordList.size(); i++)
        {
            if (wordList.indexOf("a") != -1 || wordList.indexOf("e") != -1 || wordList.indexOf("i") != -1 || wordList.indexOf("0") != -1)
            {
                wordList.remove(i);
                i--;
            }
    ...
    public static ArrayList<String> parseWordsAndReverse(String sentence) {
        ...
        String delimiter = " ";
        while (sentence.indexOf(delimiter) != -1) {
            int endIdx = sentence.indexOf(delimiter);
            String word = sentence.substring(0, endIdx);
            list.add(0, word);
            sentence = sentence.substring(endIdx + 1, sentence.length());
        }
        list.add(0, sentence);
        return list;
    }
    ...
    public static ArrayList<Integer> modes(int[] numList)
    {
        ...
        for (int i = 0; i < numList.length; i++)
        {
            ...
            else if (times == maxTimes && maxTimes != 1)
            {
                modeList.add(numList[i]);
            }
        }
